...
def export_tests_csv(request):
    ...
    writer.writerow(['מס׳', 'קוד בדיקה', 'שם בדיקה', 'שמות נוספים', 'פירוט בדיקות המשך', 'הכנת החולה לפני הדיגום', 'סוג הדגימה', 'מגבלות הבדיקה', 'כלי קיבול לדגימה ', 'צבע פקק ', 'נפח דגימה מינימלי נדרש', 'נפח דם נדרש למקבצי בדיקות שונים', 'תנאי לקיחה ושימור טרם שינוע',
                    'תנאי שינוע', 'זמן מירבי מדיגום עד הגעה למעבדה', 'מען למשלוח בדיקות', 'מטרת הבדיקה', 'מידע קליני', 'שיטת ביצוע הבדיקה', 'משך הזמן מקבלת הדגימה ועד הפצת תשובה ', 'האם הבדיקה מבוצעת גם בערבים וסופ"ש? ', 'בקרת איכות חיצונית', 'קוד מחיר משרד הבריאות', 'הערות', 'מעבדה מבצעת'])

    x = Session.displayedTest()
    tests = x.values_list('id', 'testcode', 'name', 'addnames', 'posttests', 'preparment', 'kind', 'limitations', 'vessel', 'color', 'volume', 'bloodamount', 'pretransportconditions',
                          'transportconditions', 'maxtime', 'addressee', 'testpurpose', 'clinicalinfo', 'method', 'processingtime', 'specialdays', 'outtermonitoring', 'pricecode', 'comments')
...
def export_tests_xslx(request):
    ...
    columns = ['מס׳', 'קוד בדיקה', 'שם בדיקה', 'שמות נוספים', 'פירוט בדיקות המשך', 'הכנת החולה לפני הדיגום', 'סוג הדגימה', 'מגבלות הבדיקה', 'כלי קיבול לדגימה ', 'צבע פקק ', 'נפח דגימה מינימלי נדרש', 'נפח דם נדרש למקבצי בדיקות שונים', 'תנאי לקיחה ושימור טרם שינוע',
               'תנאי שינוע', 'זמן מירבי מדיגום עד הגעה למעבדה', 'מען למשלוח בדיקות', 'מטרת הבדיקה', 'מידע קליני', 'שיטת ביצוע הבדיקה', 'משך הזמן מקבלת הדגימה ועד הפצת תשובה ', 'האם הבדיקה מבוצעת גם בערבים וסופ"ש? ', 'בקרת איכות חיצונית', 'קוד מחיר משרד הבריאות', 'הערות', 'מעבדה מבצעת']
...
class searchView(View):
    ...
    def get(self, request, letter=""):
        ...
        if (letter != ""):
            tests = getdistinct(Test.objects.filter(name__startswith=encoding.smart_str(
                letter, encoding='utf-8', strings_only=False, errors='strict')))
